...
| doneCancelSize | int | '确定取消按钮大小' | android only |
| titleSize | int | '标题文字大小' | android only |
| contentSize | int | '内容文字大小' | android only |
| cancelEnabel | boolean | '是否能取消' | android only |
| isCenterLable | boolean | '是否只显示中间的label' | android only |
...
| lineSpacingMultiplier | float | '条目间距倍数 默认1.6' | android only |
| dividerType | string | '分割线类型：fill or wrap' | android only |
| isCyclic | boolean | '滚轮是否循环' | android only |
